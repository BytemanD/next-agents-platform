...
from nap.task_worker.vector.models import RetrivalDoc
...
class ChromadbDriver:
    ...
    def _get_collection(self, collection_name: Optional[str] = None):
        return self.client.get_or_create_collection(
            name=collection_name or context.getvar("project") or DEFAULT_COLLECTION_NAME
        )
    ...
    def import_file(self, doc: Knowledge, content: str):
        ...
        logger.info("add doc {} to collection {}", doc.path, collection.name)
        collection.add(
            ids=[doc.uuid],
            documents=[content],
